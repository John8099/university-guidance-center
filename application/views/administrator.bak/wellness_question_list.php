    ...
    <tr>
        ...
        <td>
            <?php if($row->Status=='Disable') : ?>
                <a href="<?=site_url().'administrator/wellness_check_question_status/'.$row->QuestionID.'/Enable';?>" class="btn btn-outline-warning btn-sm" title="Set Enable" style="width: 100px;">Set Enable</a>
            <?php else: ?>
                <a href="<?=site_url().'administrator/wellness_check_question_status/'.$row->QuestionID.'/Disable';?>" class="btn btn-outline-warning btn-sm" title="Set Disable" style="width: 100px;">Set Disable</a>
            <?php endif; ?>
            <?php if ($row->Status=='Disable'): ?>
            ...
            <a href="<?=site_url().'administrator/wellness_check_question_confirm/'.$row->QuestionID.'/'.$row->WellnessCheckID;?>" class="btn btn-outline-danger btn-sm" title="Delete" style="width: 100px;">Delete</a>
            <?php else: ?>
...
if ($this->session->flashdata('isPublish')=='true') {
    ...
    if($tblwellnessquestion->num_rows()<>0) {
        ...
        echo $this->routines->callSweetAlertYesNo("Data was successfully saved","Do you want to publish this on email?","info","Quit","Publish",site_url().'administrator/wellness_question_publish');
        }
    }
}
...
if ($this->session->flashdata('isQuestionDelete')=='true') {
    echo $this->routines->callSweetAlertYesNo("Are you sure you want to delete this question?","","warning","No","Yes",site_url().'administrator/wellness_check_question_delete/'.$this->uri->segment(3).'/'.$this->uri->segment(4),site_url().'administrator/wellness_question_list');
}
