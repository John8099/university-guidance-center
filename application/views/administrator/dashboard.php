...
<?php
$TotalAppointments = 0;
$TotalAppointments = $this->db->query("SELECT COUNT(AppointmentID) AS Total FROM tblappointment WHERE Status!='Completed' AND CollegeID='" . $this->session->userdata('CollegeID') . "';")->row()->Total;
$CompletedAppointments = 0;
$CompletedAppointments = $this->db->query("SELECT COUNT(AppointmentID) AS Total FROM tblappointment WHERE Status='Completed' AND CollegeID='" . $this->session->userdata('CollegeID') . "';")->row()->Total;
$TotalStudents = 0;
$TotalStudents = $this->db->query("SELECT COUNT(UserID) AS Total FROM tbluser WHERE UserType='Student' AND CollegeID='" . $this->session->userdata('CollegeID') . "';")->row()->Total;
?>
<div class="row">
  <div class="col-md-12">
    <div class="card">
        ...
        <table id="assessmentTable" class="table table-hover">
          ...
          <tbody>
            ...
            $assessmentQ = $this->db->query("SELECT 
                                        ...
                                        WHERE u.CollegeID = '" . $this->session->userdata('CollegeID') . "'
                                        ");
...
<script>
  ...
  function saveRemarks(remarks, resultId) {
    swal.showLoading();
    $.post(
      `<?= site_url() . 'administrator/save_remarks/' ?>`, {
        remark: remarks,
        resultId: resultId,
