  ...
  <script>
    ...
    <div class="row">
      ...
      if (document.querySelector("#barChart")) {
        $.get(
          `<?= site_url() . 'superadmin/get_bar_data/' ?>`,
          (res, status) => {
        ...
        $("#barCourseFilter").on("change", function(e) {
          ...
          $.get(
            `<?= site_url() . 'superadmin/get_bar_data?filterBy=course&&filterByValue=' ?>${value}`,
            (res, status) => {
              barData = JSON.parse(res)
              let filterBarData = months.map((d) => {
                if (barData.some((a) => a.SelectedDate === d)) {
        ...
        $("#barGenderFilter").on("change", function(e) {
          const value = e.target.value
          $.get(
            `<?= site_url() . 'superadmin/get_bar_data?filterBy=gender&&filterByValue=' ?>${value}`,
            (res, status) => {
              barData = JSON.parse(res)
              let filterBarData = months.map((d) => {
                if (barData.some((a) => a.SelectedDate === d)) {
        ...
        $("#barStudentYearFilter").on("change", function(e) {
          const value = e.target.value
          $.get(
            `<?= site_url() . 'superadmin/get_bar_data?filterBy=studentYear&&filterByValue=' ?>${value}`,
            (res, status) => {
              barData = JSON.parse(res)
              let filterBarData = months.map((d) => {
                if (barData.some((a) => a.SelectedDate === d)) {
      ...
      if (document.querySelector("#lineChart")) {
        $.get(
          `<?= site_url() . 'superadmin/get_line_data/' ?>`,
          (res, status) => {
        ...
        $("#lineMonthFilter").on("change", function(e) {
          const value = e.target.value
          $.get(
            `<?= site_url() . 'superadmin/get_line_data?filterBy=month&&filterByValue=' ?>${value}`,
            (res, status) => {
              const d = new Date();
              lineData = JSON.parse(res)
              console.log(lineData)
        ...
        $("#lineCourseFilter").on("change", function(e) {
          const value = e.target.value
          $.get(
            `<?= site_url() . 'superadmin/get_line_data?filterBy=course&&filterByValue=' ?>${value}`,
            (res, status) => {
              const d = new Date();
              lineData = JSON.parse(res)
              console.log(lineData)
