              ...
              <tr>
                ...
                <td>
                  <?php if ($row->Status == 'Disable') : ?>
                    <a href="<?= site_url() . 'superadmin/wellness_check_status/' . $row->WellnessCheckID . '/Enable'; ?>" class="btn btn-outline-warning btn-sm" title="Set Enable" style="width: 100px; margin: .25rem">Set Enable</a>
                  <?php else : ?>
                    <a href="<?= site_url() . 'superadmin/wellness_check_status/' . $row->WellnessCheckID . '/Disable'; ?>" class="btn btn-outline-warning btn-sm" title="Set Disable" style="width: 100px; margin: .25rem">Set Disable</a>
                  <?php endif; ?>
                  <?php if ($row->Status == 'Disable') : ?>
                    <a href="<?= site_url() . 'superadmin/wellness_check/' . $row->WellnessCheckID; ?>" class="btn btn-outline-primary btn-sm" title="Edit" style="width: 100px; margin: .25rem">Edit</a>
                    <a href="<?= site_url() . 'superadmin/wellness_checks_delete/' . $row->WellnessCheckID; ?>" class="btn btn-outline-danger btn-sm" title="Delete" style="width: 100px; margin: .25rem">Delete</a>
                  <?php else : ?>
